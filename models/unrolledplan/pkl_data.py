...
def main():
    ...
    lens = []
    for _dir in dirs:
        txt_file, qt_txt_file = glob.glob(os.path.join(_dir, '*txt'))
        if 'qt.txt' in txt_file:
            txt_file, qt_txt_file = qt_txt_file, txt_file
        img_files = glob.glob(os.path.join(_dir, '*p'))
        img_files = sorted(img_files, key=lambda x: int(x.split(os.path.sep)[-1].split(".")[0]))
        f = open(txt_file, 'r')
        g = open(qt_txt_file, 'r')
        _txt_data = [line.strip().split() for line in f]#[1:]
        _qt_txt_data = [line.strip().split() for line in g]#[1:]
        f.close()
        g.close()
        _img_data = []
        ...
        assert len(_qt_txt_data) == len(_img_data), "{}_{}".format(len(_qt_txt_data), len(_img_data))
        _txt_data = np.array(_txt_data, dtype=np.float32)
        _qt_txt_data = np.array(_qt_txt_data, dtype=np.float32)

        txt_data.append(_txt_data)
        qt_txt_data.append(_qt_txt_data)
        img_data.append(_img_data)
        lens.append(len(_img_data))

    if not os.path.exists(args.save_dir):
        os.makedirs(args.save_dir)
    pickle.dump([txt_data, qt_txt_data, img_data, lens],
